...
export function DismissibleStickyBanner({ adKey }: DismissibleStickyBannerProps) {
  ...
  const handleRenderStateChange = useCallback((hasRenderedContent: boolean) => {
    setHasRenderedAd(hasRenderedContent);
  }, []);
  ...
  return (
    <div className={`sticky top-20 z-20 ${hasRenderedAd ? "py-2" : ""}`}>
      <div className={`relative mx-auto max-w-4xl ${hasRenderedAd ? "pr-10" : ""}`}>
        <AdBanner
          type="banner-320x50"
          adKey={adKey}
          eager
